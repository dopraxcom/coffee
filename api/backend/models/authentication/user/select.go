...
func LoginSuperUserAdmin(admin *models.Admin) (bool, *models.Admin) {
	...
	db := database.CreateCon()
	err := db.QueryRow("select id,firstname,lastname,username,email,role_id,created_at,status from `ico_user_admin` where username='"+admin.UserName+"' and password='"+hex.EncodeToString(hashPassword[:])+"';").
		Scan(&admin.ID, &admin.FirstName, &admin.LastName, &admin.UserName, &admin.Email, &admin.Role.ID, &admin.CreatedAt, &admin.Status)
